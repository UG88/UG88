![Typing SVG](https://readme-typing-svg.herokuapp.com?size=28&duration=3000&color=00F7FF&center=true&vCenter=true&width=800&lines=UG88+-+Systems+Developer;Minecraft+%7C+Hosting+%7C+SaaS;Building+Scalable+Systems)

# <div align="center">✨ UG88 ✨</div>

<div align="center">

<img src="https://readme-typing-svg.herokuapp.com?font=Inter&weight=600&size=30&duration=3200&pause=800&color=7C3AED&center=true&vCenter=true&width=900&lines=Systems+Developer;Minecraft+Engineer;Hosting+Architect;Building+Scalable+Digital+Systems" alt="Typing animation" />

</div>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=220&text=UG88&fontSize=58&fontAlign=50&fontAlignY=38&desc=Animated%20Glass%20UI%20Profile&descAlign=50&descAlignY=58&color=gradient&customColorList=12,20,24,30" alt="header" />
</div>

---

<div align="center">

### 🫧 Modern systems. Clean design. Scalable builds.

</div>

<table align="center">
<tr>
<td width="50%" valign="top">

## 🪟 About Me

```yaml
name: UG88
role: Systems Developer
focus:
  - Minecraft plugin ecosystems
  - Hosting platforms & infrastructure
  - Full-stack web systems
  - Automation & AI-assisted workflows
style:
  - Clean architecture
  - Performance-first
  - Premium UI/UX
  - Production-ready builds
```

I design and build **modern, scalable systems** with a strong focus on **Minecraft infrastructure, hosting platforms, web products, and polished user experiences**.

I like building things that are not only functional, but also **fast, secure, and visually refined**.

</td>
<td width="50%" valign="top">

## 🌌 Current Focus

* ⚙️ Advanced Minecraft plugin development
* 🌐 Hosting ecosystem & control panels
* 🧠 AI-assisted productivity systems
* 🎨 Glassmorphism-inspired UI/UX
* 🔒 Secure backend architecture
* 🚀 Scalable deployment workflows

<br />

## 🧩 Motto

> **Build. Scale. Refine. Repeat.**

</td>
</tr>
</table>

---

## ✨ Glass UI Tech Stack

<div align="center">

<img src="https://skillicons.dev/icons?i=java,nodejs,python,react,ts,js,html,css,mysql,docker,linux,nginx,git,github,vscode" />

</div>

<br />

<div align="center">

![Java](https://img.shields.io/badge/Java-Glass_UI-1f1f2e?style=for-the-badge\&logo=openjdk\&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-Backend-1f1f2e?style=for-the-badge\&logo=nodedotjs\&logoColor=white)
![Python](https://img.shields.io/badge/Python-Automation-1f1f2e?style=for-the-badge\&logo=python\&logoColor=white)
![React](https://img.shields.io/badge/React-Frontend-1f1f2e?style=for-the-badge\&logo=react\&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-Data-1f1f2e?style=for-the-badge\&logo=mysql\&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-Deployments-1f1f2e?style=for-the-badge\&logo=docker\&logoColor=white)

</div>

---

## 📊 Animated Profile Stats

<div align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=ug88&show_icons=true&theme=transparent&hide_border=true&rank_icon=github" />
  <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=ug88&layout=compact&theme=transparent&hide_border=true" />
</div>

<div align="center">
  <img height="170" src="https://streak-stats.demolab.com?user=ug88&theme=transparent&hide_border=true" />
</div>

---

## 🚀 Featured Areas

<div align="center">

| Area                 | What I Build                                                            |
| -------------------- | ----------------------------------------------------------------------- |
| 🎮 Minecraft Systems | Plugins, network tools, server ecosystems, Velocity/Paper/Folia support |
| ☁️ Hosting Platforms | Panels, provisioning flows, VPS infrastructure, deployment logic        |
| 🌐 Web Products      | Frontend experiences, backend APIs, auth flows, dashboards              |
| 🤖 Automation        | AI-enhanced workflows, helper systems, productivity tooling             |
| 🎨 UI/UX             | Modern interfaces, glassmorphism concepts, premium visual design        |

</div>

---

## 🌍 Connect

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-ug88-181717?style=for-the-badge\&logo=github)](https://github.com/ug88)
[![Modrinth](https://img.shields.io/badge/Modrinth-UG88-00AF5C?style=for-the-badge\&logo=modrinth\&logoColor=white)](https://modrinth.com/user/UG88)

</div>

---

## 🫧 Optional Add-ons

You can make this profile feel even more premium by adding:

* A custom **banner image** with your branding
* A **snake contribution animation**
* Pinned repositories with matching thumbnails
* Repo badges with consistent dark-glass styling
* A visitor counter if you want profile traffic visibility

---

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&section=footer&height=120&color=gradient&customColorList=12,20,24,30" alt="footer" />
</div>
